...
{   
    ...
    Vector3f c0 = v_ab.cross(v_ad);

    Vector3f v_bc = p_c - p_b;
    Vector3f v_bd = p_d - p_b;
    Vector3f c1 = v_bc.cross(v_bd);

    Vector3f v_ca = p_a - p_c;
    Vector3f v_cd = p_d - p_c;
    Vector3f c2 = v_ca.cross(v_cd);

    if (c0[2] > 0.0 && c1[2] > 0.0 && c2[2] > 0.0) {
        return true;
    }
    if (c0[2] < 0.0 && c1[2] < 0.0 && c2[2] < 0.0) {
        return true;
...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
    ...
    float min_x = min3(v[0][0], v[1][0], v[2][0]);
    float max_x = max3(v[0][0], v[1][0], v[2][0]);
    float min_y = min3(v[0][1], v[1][1], v[2][1]);
    float max_y = max3(v[0][1], v[1][1], v[2][1]);

    #undef min3
    #undef max3

    int left = (int)floor(min_x);
